...
**Acceptance Criteria：**
- [ ] 支持 PDF 和 Word（.docx）格式拖拽上传
- [ ] 自动提取结构化字段：基本信息、教育经历、工作经历、项目经历、技能列表
- [ ] 多份简历间智能去重（同一公司同一岗位只保留最新版）
- [ ] 生成初始 Git 树：Master 主干 + 自动推断的方向分支
- [ ] 解析失败时保留原始文本并标注「需人工确认」
- [ ] 单份简历解析 ≤ 5s（10 页以内）
...
**Acceptance Criteria：**
- [ ] React Flow 渲染可缩放、可拖拽的树状画布
- [ ] 三种节点形态区分：主干（圆形/青色）、方向分支（圆角矩形/紫色）、公司节点（矩形/橙色）
- [ ] 点击节点进入编辑或预览
- [ ] 面包屑显示当前路径（Master → 安全岗 → Tencent-研究员）
- [ ] 支持手动新建分支/节点
- [ ] 节点数量 ≤ 50 时画布渲染 ≤ 100ms
...
**Acceptance Criteria：**
- [ ] 支持 PNG、JPG 格式拖拽上传
- [ ] 多模态 LLM 提取结构化字段：技术栈、硬技能、软技能、加分项
- [ ] 右栏 JD 卡片展示，支持编辑修正
- [ ] 截图提取失败时提示重新上传
- [ ] 单张截图提取 ≤ 10s
